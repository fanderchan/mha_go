...
- 管理 SQL 凭据（`sql.user` / `sql.password_ref`）与复制源凭据（`sql.replication_user` / `sql.replication_password_ref`）分离；后者写入 `SOURCE_USER` / `SOURCE_PASSWORD`
...
- 不带 `--dry-run` 的 `failover-execute` 使用 `MySQLActionRunner`：`writer_endpoint.kind` 为 `vip`/`proxy` 时先执行 endpoint precheck（确认切换命令存在，并运行可选 `precheck_command`）；按 `fencing.steps` 对旧主做隔离，默认 required `read_only`（旧主可连则 `super_read_only`/`read_only`，旧主已在拓扑中标记为 dead 且不可连则跳过）；SQL 可达的补数步骤将候选指向 donor 后 `WAIT_FOR_EXECUTED_GTID_SET`；旧主 SQL 已死但 SSH 可达时，binlog salvage 通过 `mysqlbinlog` 流式读取旧主本地 binlog，并用 GTID include/exclude 过滤后应用到候选主；候选主 `STOP REPLICA` / `RESET REPLICA ALL` / 关闭只读后提升；只重指向规划阶段可达的从库，普通 dead 从库会跳过并留待后续 rejoin；写入口切换通过 `writer_endpoint.command` 或环境变量 `MHA_WRITER_ENDPOINT_COMMAND` 执行外部脚本；可选 `verify_command` 验证 endpoint；`verify-cluster` 用 SQL 巡检新主可写且可达从库指向新主
...
- 执行前，如果候选副本当前可写，switchover 会校验它是否存在旧主没有的 errant GTID。候选副本即使当前可写，只要它的 GTID set 仍被旧主 GTID set 包含，就允许继续。
- 执行时，`LockCandidate` 会先把候选副本设为只读，再锁旧主，关闭候选副本继续产生本地 errant GTID 的窗口。
- `LockOldPrimary` 之后，`WaitCandidateCatchUp` 不信任只读后立刻读取的一次 `gtid_executed` 快照。它会等待旧主上的活动 InnoDB 写事务排空，要求旧主 GTID set 连续多次采样保持稳定，然后才让候选主等待这个最终 GTID set。
...
2. 旧主不可 SQL 访问但 agent/SSH 可访问：读取本地 binlog 并抽取差异 GTID 事务。当前 SSH 实现会在远端对配置的本地 binlog 执行 `mysqlbinlog`，再用 manager 本地 `mysql` 客户端应用到候选主。
...
- agent salvage 强化和 SSH binlog salvage 集成测试
